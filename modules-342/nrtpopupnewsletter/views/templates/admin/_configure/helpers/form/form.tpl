...
{block name="input"}
	{if $input.type == 'background_image'}
                ...
                <a href="{__PS_BASE_URI__}{basename(_PS_ADMIN_DIR_)}/filemanager/dialog.php?type=1&field_id={$input.name}" class="btn btn-default iframe-upload"  data-input-name="{$input.name}" type="button">{l s='Background image selector' mod='nrtpopupnewsletter'} <i class="icon-angle-right"></i></a>
	{else}
		{$smarty.block.parent}
    {/if}
{/block}
