...
class SizeGuideModel extends ObjectModel
{
	...
	public static $definition = array(
		'table' => 'nrtsizechart_guides',
		'primary' => 'id_nrtsizechart_guides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'nrtsizechart` (`id_shop`, `id_nrtsizechart_guides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'nrtsizechart`
			WHERE `id_nrtsizechart_guides` = '.(int)$this->id
		);

		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'nrtsizechart_product`
	...
	public static function getProductGuide($id_product)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('
			SELECT `id_guide`
			FROM '._DB_PREFIX_.'nrtsizechart_product 
	...
	public static function assignProduct($id_product, $id_guide)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'nrtsizechart_product` (`id_product`, `id_guide`) 
