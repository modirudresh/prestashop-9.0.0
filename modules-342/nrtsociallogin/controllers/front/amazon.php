...
class NrtSocialLoginAmazonModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
		...
		if (!Tools::strlen($settings['amazon']['client_id']) || !Tools::strlen($settings['amazon']['client_secret'])) {
		...
		try {
			$hybridauth = new Hybridauth(
				[
					'callback' => $this->module->_getUrlCallback('amazon'),
					'providers' => [
						'Amazon' => [
							'enabled' => true,
							'keys' => [ 'id' => $settings['amazon']['client_id'], 'secret' => $settings['amazon']['client_secret']]
						]
					],
				]
			);

			$authenticate = $hybridauth->authenticate( 'Amazon' );
