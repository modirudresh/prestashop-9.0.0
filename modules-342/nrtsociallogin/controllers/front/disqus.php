...
class NrtSocialLoginDisqusModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
		...
		if (!Tools::strlen($settings['disqus']['client_id']) || !Tools::strlen($settings['disqus']['client_secret'])) {
		...
		try {
			$hybridauth = new Hybridauth(
				[
					'callback' => $this->module->_getUrlCallback('disqus'),
					'providers' => [
						'Disqus' => [
							'enabled' => true,
							'keys' => [ 'id' => $settings['disqus']['client_id'], 'secret' => $settings['disqus']['client_secret']]
						]
					],
				]
			);

			$authenticate = $hybridauth->authenticate( 'Disqus' );
