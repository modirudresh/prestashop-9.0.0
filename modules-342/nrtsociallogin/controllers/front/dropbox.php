...
class NrtSocialLoginDropboxModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
		...
		if (!Tools::strlen($settings['dropbox']['client_id']) || !Tools::strlen($settings['dropbox']['client_secret'])) {
		...
		try {
			$hybridauth = new Hybridauth(
				[
					'callback' => $this->module->_getUrlCallback('dropbox'),
					'providers' => [
						'Dropbox' => [
							'enabled' => true,
							'keys' => [ 'id' => $settings['dropbox']['client_id'], 'secret' => $settings['dropbox']['client_secret']]
						]
					],
				]
			);

			$authenticate = $hybridauth->authenticate( 'Dropbox' );
