...
class NrtSocialLoginFacebookModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
		...
		if (!Tools::strlen($settings['facebook']['app_id']) || !Tools::strlen($settings['facebook']['app_secret'])) {
		...
		try {
			$hybridauth = new Hybridauth(
				[
					'callback' => $this->module->_getUrlCallback('facebook'),
					'providers' => [
						'Facebook' => [
							'enabled' => true,
							'keys' => [ 'id' => $settings['facebook']['app_id'], 'secret' => $settings['facebook']['app_secret']],
							'scope'    => 'email'
						],
					],
				]
			...
			$authenticate = $hybridauth->authenticate( 'Facebook' );
