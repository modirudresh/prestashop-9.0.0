...
class NrtSocialLoginFoursquareModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
		...
		if (!Tools::strlen($settings['foursquare']['client_id']) || !Tools::strlen($settings['foursquare']['client_secret'])) {
		...
		try {
			$hybridauth = new Hybridauth(
				[
					'callback' => $this->module->_getUrlCallback('foursquare'),
					'providers' => [
						'Foursquare' => [
							'enabled' => true,
							'keys' => [ 'id' => $settings['foursquare']['client_id'], 'secret' => $settings['foursquare']['client_secret']]
						]
					],
				]
			);

			$authenticate = $hybridauth->authenticate( 'Foursquare' );
