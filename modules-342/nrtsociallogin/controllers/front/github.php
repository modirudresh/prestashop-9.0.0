...
class NrtSocialLoginGithubModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
		...
		if (!Tools::strlen($settings['github']['client_id']) || !Tools::strlen($settings['github']['client_secret'])) {
		...
		try {
			$hybridauth = new Hybridauth(
				[
					'callback' => $this->module->_getUrlCallback('github'),
					'providers' => [
						'GitHub' => [
							'enabled' => true,
							'keys' => [ 'id' => $settings['github']['client_id'], 'secret' => $settings['github']['client_secret']]
						]
					],
				]
			);

			$authenticate = $hybridauth->authenticate( 'GitHub' );
