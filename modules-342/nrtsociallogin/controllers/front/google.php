...
class NrtSocialLoginGoogleModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
		...
		if (!Tools::strlen($settings['gplus']['client_id']) || !Tools::strlen($settings['gplus']['client_secret'])) {
		...
		try {
			$hybridauth = new Hybridauth(
				[
					'callback' => $this->module->_getUrlCallback('google'),
					'providers' => [
						'Google' => [
							'enabled' => true,
							'keys' => [ 'id' => $settings['gplus']['client_id'], 'secret' => $settings['gplus']['client_secret']]
						]
					],
				]
			);

			$authenticate = $hybridauth->authenticate( 'Google' );
