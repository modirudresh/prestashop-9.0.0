...
class NrtSocialLoginInstagramModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
		...
		if (!Tools::strlen($settings['insta']['client_id']) || !Tools::strlen($settings['insta']['client_secret'])) {
		...
		try {
			$hybridauth = new Hybridauth(
				[
					'callback' => $this->module->_getUrlCallback('instagram'),
					'providers' => [
						'Instagram' => [
							'enabled' => true,
							'keys' => [ 'id' => $settings['insta']['client_id'], 'secret' => $settings['insta']['client_secret']]
						]
					],
				]
			);

			$authenticate = $hybridauth->authenticate( 'Instagram' );
