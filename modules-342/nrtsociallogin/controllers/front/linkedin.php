...
class NrtSocialLoginLinkedinModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
		...
		if (!Tools::strlen($settings['linked']['client_id']) || !Tools::strlen($settings['linked']['client_secret'])) {
		...
		try {
			$hybridauth = new Hybridauth(
				[
					'callback' => $this->module->_getUrlCallback('linkedin'),
					'providers' => [
						'LinkedIn' => [
							'enabled' => true,
							'keys' => [ 'id' => $settings['linked']['client_id'], 'secret' => $settings['linked']['client_secret']]
						]
					],
				]
			);

			$authenticate = $hybridauth->authenticate( 'LinkedIn' );
