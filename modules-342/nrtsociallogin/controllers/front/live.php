...
class NrtSocialLoginLiveModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
		...
		if (!Tools::strlen($settings['live']['client_id']) || !Tools::strlen($settings['live']['client_secret'])) {
		...
		try {
			$hybridauth = new Hybridauth(
				[
					'callback' => $this->module->_getUrlCallback('live'),
					'providers' => [
						'WindowsLive' => [
							'enabled' => true,
							'keys' => [ 'id' => $settings['live']['client_id'], 'secret' => $settings['live']['client_secret']]
						]
					],
				]
			);

			$authenticate = $hybridauth->authenticate( 'WindowsLive' );
