...
class NrtSocialLoginPaypalModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
		...
		if (!Tools::strlen($settings['pay']['client_id']) || !Tools::strlen($settings['pay']['client_secret'])) {
		...
		try {
			$hybridauth = new Hybridauth(
				[
					'callback' => $this->module->_getUrlCallback('paypal'),
					'providers' => [
						'Paypal' => [
							'enabled' => true,
							'keys' => [ 'id' => $settings['pay']['client_id'], 'secret' => $settings['pay']['client_secret']]
						]
					],
				]
			);

			$authenticate = $hybridauth->authenticate( 'Paypal' );
