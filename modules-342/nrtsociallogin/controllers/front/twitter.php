...
class NrtSocialLoginTwitterModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
		...
		if (!Tools::strlen($settings['twitter']['client_id']) || !Tools::strlen($settings['twitter']['client_secret'])) {
		...
		try {
			$hybridauth = new Hybridauth(
				[
					'callback' => $this->module->_getUrlCallback('twitter'),
					'providers' => [
						'Twitter' => [
							'enabled' => true,
							'keys' => [ 'id' => $settings['twitter']['client_id'], 'secret' => $settings['twitter']['client_secret']]
						]
					],
				]
			);

			$authenticate = $hybridauth->authenticate( 'Twitter' );
