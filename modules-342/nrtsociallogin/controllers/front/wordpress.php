...
class NrtSocialLoginWordpressModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
		...
		if (!Tools::strlen($settings['wordpress']['client_id']) || !Tools::strlen($settings['wordpress']['client_secret'])) {
		...
		try {
			$hybridauth = new Hybridauth(
				[
					'callback' => $this->module->_getUrlCallback('wordpress'),
					'providers' => [
						'WordPress' => [
							'enabled' => true,
							'keys' => [ 'id' => $settings['wordpress']['client_id'], 'secret' => $settings['wordpress']['client_secret']]
						]
					],
				]
			);

			$authenticate = $hybridauth->authenticate( 'WordPress' );
