...
SmartTree.prototype =
{
			...
			function ()
			{
				...
				else
				{
					...
					if (load_tree)
					{
						...
						$.get(
							'index.php',
							{controller:'AdminBlogPost',token:currentToken,action:'getCategoryTree',type:idTree,ajax: 1,category:category,inputName:name,useCheckBox:useCheckBox},
							function(content)
							{
	...
	expandAll : function($speed)
	{
		...
		if (typeof(idTree) != 'undefined' && !$('#'+idTree).hasClass('full_loaded'))
		{
			...
			$.get(
				'index.php',
				{controller:'AdminBlogPost',token:currentToken,action:'getCategoryTree',ajax: 1,type:idTree,fullTree:1,selected:selected, inputName:name,useCheckBox:useCheckBox},
				function(content)
				{
					$('#'+idTree).html(content);
					$('#'+idTree).smartcattree('init');
