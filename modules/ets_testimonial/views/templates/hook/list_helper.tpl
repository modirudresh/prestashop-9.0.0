    ...
    <div class="panel-heading">{*if isset($icon) && $icon}<i class="{$icon|escape:'html':'UTF-8'}"></i>&nbsp;{/if*}{$title nofilter}
        ...
        <span class="panel-heading-action">
            {if isset($show_add_new) && $show_add_new}            
                <a class="list-toolbar-btn add_new_link" data-placement="top" data-toggle="tooltip" data-tooltip="{l s='Add new' mod='ets_testimonial'}" data-original-title="{l s='Add new' mod='ets_testimonial'}" href="{if isset($link_new)}{$link_new|escape:'html':'UTF-8'}{else}{$currentIndex|escape:'html':'UTF-8'}{/if}">
                    <span>
                                        ...
                                        <td class="text-right">                            
                                            <div class="btn-group-action">
                                                <div class="btn-group pull-right">
                                                        ...
                                                        {/if}
                                                        {if $actions[0]=='delete'}
                                                            <a class="btn btn-default btn-delete" onclick="return confirm('{l s='Do you want to delete this item?' mod='ets_testimonial' js=1}');" href="{$currentIndex|escape:'html':'UTF-8'}&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}&del=yes"><i class="icon-trash fa fa-trash"></i> {l s='Delete' mod='ets_testimonial'}</a>
                                                        {/if}
                                                                ...
                                                                {foreach from=$actions item='action' key='key'}
                                                                    {if $key!=0}
                                                                        {if $action=='delete' && (!isset($row.view_order_url) || (isset($row.view_order_url) && !$row.view_order_url) )}
                                                                            <li><a class="btn btn-default btn-delete" onclick="return confirm('{if $name=='mp_front_products' || $name=='mp_products'}{l s='Do you want to delete this product?' mod='ets_testimonial'}{else}{l s='Do you want to delete this item?' mod='ets_testimonial'}{/if}');" href="{$currentIndex|escape:'html':'UTF-8'}&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}&del=yes{if isset($row.type)}&type={$row.type|escape:'html':'UTF-8'}{/if}"><i class="fa fa-trash icon-trash"></i> {l s='Delete' mod='ets_testimonial'}</a></li>
                                                                        {/if}
                                                                        {if $action=='view'}
                                                                            {if isset($row.child_view_url) && $row.child_view_url}
